...
def check_points_present(list1, list2, tolerance=0):
...
def Normal_point_from_surface(point1, point2, distance=3.0):
    """
...
def rotate_drone_toward_normal(client, current_position, point1, duration=3.0, vehicle_name="Scout_drone"):
...
def get_two_furthest_points_from_drone(points_list):
    """
    Find two points with the furthest distance from the drone at origin (0,0,0).
...
def save_global_ned_points_to_ply(points, ply_file):
    """
    Save global NED points into a .ply file.

    Args:
        points: list of (x, y, z) coordinates or numpy array of shape (N,3)
        ply_file: path to save the .ply file
    """
    if len(points) == 0:
